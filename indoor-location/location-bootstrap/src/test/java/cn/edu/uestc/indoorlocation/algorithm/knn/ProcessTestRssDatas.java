...
public class ProcessTestRssDatas implements Iterable<MRSS>{

	private static int RSS_LIMIT = 25;
	private String filename;
//	private Random RANDOM = new Random();
	public ProcessTestRssDatas(String filename) {
		this.filename = filename;
	}
	@Override
	public Iterator<MRSS> iterator() {
...
//				obj.put("RSS", list.get(list.size()/2));
		...
		public Object next() {
			...
			Point point = new Point(Integer.parseInt(p[0]), 
					Integer.parseInt(p[1]));
			int n = Integer.parseInt(scanner.nextLine().trim());
			
//			int chosen = RANDOM.nextInt(n);
//			System.out.println(Arrays.toString(p) + " n: " + n);
...
//			scanner.nextLine();
			...
			return new MRSS(point, generateJson(map));
		}
	}
}
